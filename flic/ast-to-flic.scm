...
(define (init-flic-var var value toplevel?)
  ...
  (let ((inline (lookup-annotation var '|Inline|))
	(always (lookup-annotation var '|AlwaysInline|)))
    (when (or inline always)
      (setf (var-inline? var) '#t)
      (when always
	(setf (var-always-inline? var) '#t))
	...
	(cond ((is-type? 'valdef (car decls))
		 ...
		 (values
		   ...
		   (if (null? more-bindings)
		       newerbody
		       (begin
			 (munge-specializers more-bindings)
...
;;; For generic functions with specialized versions, we need to inline
;;; the call to the generic version inside the body of the specialized
;;; version, and save the argument patterns away for the optimizer to
;;; mess with later.

(define (munge-specializers bindings)
  (dolist (var bindings)
    (let ((specializers  (var-specializers var)))
      (dolist (s specializers)
	(let* ((svar   (car s))
	       (sval   (var-value svar))
	       (lambda (if (flic-lambda? sval)
			   sval
			   (make-flic-lambda '() sval)))
	       (app    (if (flic-lambda? sval)
			   (flic-lambda-body sval)
			   sval)))
	  ;; Ignore bogus expansions.  You can get them if you try to
	  ;; make specializers for functions that don't have any
	  ;; dictionary variables.
	  (when (and (flic-app? app)
		     (flic-ref? (flic-app-fn app))
		     (eq? (flic-ref-var (flic-app-fn app)) var))
	    ;; Save the lambda for later pattern matching
	    (setf (cdr s) (copy-flic-top lambda))
	    ;; Inline the call in the specialized function body
	    (setf (flic-app-fn app) (wrap-with-let var (var-value var))))
	  )))))
