...
(define-copy-flic flic-let (object var-renamings)
  (let ((new-vars  (map (lambda (v)
			  (let ((new  (copy-temp-var (def-name v))))
			    (when (var-inline? v)
			      (setf (var-inline? new) '#t))
			    (when (var-always-inline? v)
			      (setf (var-always-inline? new) '#t))
			    (push (cons v new) var-renamings)
			    new))
...
(define-copy-flic flic-update (object var-renamings)
  (make-flic-update
    (flic-update-con object)
    (map (lambda (x) (cons (car x) (copy-flic (cdr x) var-renamings)))
	 (flic-update-slots object))
    (copy-flic (flic-update-exp object) var-renamings)))
