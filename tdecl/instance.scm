...
(define (instance->def inst-decl interface?)
 (recover-errors '#f
  (remember-context inst-decl
    (with-slots instance-decl (context class simple decls) inst-decl
      ...
      (let ((alg-def (tycon-def simple))
        ...
        (when (and (not interface?)
		   (not (algdata? (tycon-def simple))))
	  ...
	  (let ((old-inst (lookup-instance alg-def class-def))
	        (inst (new-instance class-def alg-def tyvars)))
	      (setf (ast-node-line-number inst)
		    (ast-node-line-number inst-decl))
	      ...
	      (when (and (not interface?)
			 (not (eq? old-inst '#f))
			 (not (instance-in-interface? old-inst))
			 (not (instance-special? old-inst)))
		 (signal-multiple-instance inst old-inst))
	      (when (and (not (eq? *module-name* (def-module alg-def)))
			 (not (eq? *module-name* (def-module class-def))))
		    (signal-c-t-rule-violation inst class-def alg-def))
	      (unless interface?
		 (setf (instance-runtime-var inst)
		       (make-new-var (string-append
				       (symbol->string (def-name class-def))
				       "-"
				       (symbol->string (def-name alg-def))
				       "-instance"))))
	      inst)))))))

(define (signal-datatype-required def)
  (phase-error 'datatype-required
    "Synonym types such as ~a cannot be declared as an instance of a class."
    (symbol->string (def-name def))))

(define (signal-c-t-rule-violation inst class-def alg-def)
  (phase-error/objs 'c-t-rule-violation (list inst class-def alg-def)
    "C-T rule violation.  The instance ~A must be defined in either~%~
     the module defining ~A or the module defining ~A."
     (get-object-name inst) (get-object-name class-def)
     (get-object-name alg-def)))

(define (signal-multiple-instance inst other-inst)
  (phase-error/objs 'multiple-instance (list inst other-inst)
    "The instance ~a is already defined." (get-object-name inst)))
...
(define (expand-instance-decls inst interface?)
  (when (instance-ok? inst)
    (with-slots instance (class algdata dictionary decls context tyvars) inst
     (let ((simple (**tycon/def algdata (map (function **tyvar) tyvars))))
      ...
      (if interface?
       (setf (instance-methods inst) '())
	;;; The rest of this is not needed for interface definitions
	...
	(let ((methods-used '())
          ...
          (dolist (decl decls)
            (setf methods-used
  	      (process-instance-decl decl new-instance-vars methods-used inst)))
	  ...
	  (dolist (m-v new-instance-vars)
		...
		(let ((method-body
			   ...
			   (**abort (format '#f
     "No definition for method ~A in instance ~A."
                              (get-object-name method-var) 
...
(define (process-instance-decl decl new-instance-vars methods-used inst)
  (if (valdef? decl)
      (rename-instance-decl decl new-instance-vars methods-used inst)
      (begin
...
(define (attach-method-annotation name annotations vars)
  (cond ((null? vars)
	 (signal-bad-annotated-var name annotations))
...
(define (rename-instance-decl decl new-instance-vars methods-used inst)
 (remember-context decl
  (let ((decl-vars (collect-pattern-vars (valdef-lhs decl))))
    (dolist (var decl-vars)
      (let* ((method-name (var-ref-name var))
	     (method (resolve-toplevel-name method-name))
	     (m-v (assq method new-instance-vars)))
          (cond ((memq method methods-used)
		 (signal-multiple-instance-def method-name inst))
		((eq? m-v '#f)
		 (signal-not-in-class method-name inst))
		(else
...
(define (signal-multiple-instance-def method inst)
  (phase-error/objs  'multiple-instance-def (list inst)
    "The instance declaration ~A has multiple definitions of the method ~A."
     (get-object-name inst) (symbol->string method)))

(define (signal-not-in-class method inst)
  (phase-error/objs 'not-in-class (list inst)
    "The instance declaration ~A includes a definition for ~a,~%~
...
(define (check-inst-type inst)
   (let* ((class (instance-class inst))
     ...
     (when (not (full-context-implies? inst-context alg-context))
       (signal-instance-context-needs-alg-context inst algdata))
     (dolist (super-c (class-super class))
       (let ((super-inst (lookup-instance algdata super-c)))
	 (cond ((eq? super-inst '#f)
		(signal-no-super-class-instance inst super-c))
	       (else
		(when (not (full-context-implies?
			     inst-context (instance-gcontext super-inst)))
		  (signal-instance-context-insufficient-for-super
		    inst super-inst))))))
     ))

(define (signal-instance-context-needs-alg-context inst algdata)
  (phase-error/objs 'instance-context-needs-alg-context (list inst algdata)
    "The instance context for ~A needs to include context defined~%~
     for data type ~A."
    (get-object-name inst) (get-object-name algdata)))

(define (signal-no-super-class-instance inst super-c)
  (phase-error/objs 'no-super-class-instance (list inst)
    "The instance ~A requires that the instance ~A(~A) be defined~%~
     since ~A is a superclass of ~A"
    (get-object-name inst) (get-object-name super-c)
    (get-object-name (instance-algdata inst))
    (get-object-name super-c) (get-object-name (instance-class inst)))
  (abort-compilation))

(define (signal-instance-context-insufficient-for-super inst super-inst)
  (phase-error/objs 'instance-ctxt-insufficient-for-super (list inst super-inst)
    "The context of instance ~A must include the context associated~%~
     with the super class instance ~A."
    (get-object-name inst) (get-object-name super-inst)))
