...
;(remove 'type ccl::*nx-known-declarations*)
#+ccl (set! ccl::*nx-known-declarations* (remove 'type ccl::*nx-known-declarations*))
(define-struct type
  (include ast-node))
...
(define-struct deriving-decl
  (include ast-node)
  (slots
   (constraints (type (list context)))
   (simple (type type))
   (inst-decls (type (list instance-decl)))
   ))
