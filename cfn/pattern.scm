...
(define (cfn-valdef object)
  (remember-context object
	...
	(do-cfn-pattern-def-top object) ; %%% Bug: extra defs not added
	                                ; %%% when add-dict-params not called!
  ...
  (if (null pats)
	...
	(funcall
	  (typecase pat
	    ...
	    (dynamic-pat  (function match-dynamic-pat))
...
;;; Made sure is-constructor tests are generated even for tuples - jcp
...
;;; Dynamic pats - I have no clue what I'm doing (jcp)
;;; 

(define (match-dynamic-pat pat exp pats exps alt block-name)
  (let* ((var?  (var-ref? exp))
	 (var   (if var? (var-ref-var exp) (create-temp-var 'dynexp)))
	 (uvar  (create-temp-var 'unify))
	 (sig   (dynamic-pat-sig pat))
	 (gtype (ast->gtype (signature-context sig) (signature-type sig)))
	 (pvars (map (lambda (x) (algdata-runtime-var x))
		     (dynamic-pat-runtime-vars pat))))
    ;;; uvar = coerceB dynamic type
    (push (**valdef/pat (**pat `(list ,@pvars))
			(**tuple-sel 2 1
			    (**sel (core-symbol "DSucc")
				   (**var/def uvar)
				   0)))
	  (alt-where-decls alt))
    ;;; This pulls the value out of the dynamic pattern
    (push (**valdef/pat (dynamic-pat-pat pat)
			(**tuple-sel 2 0
			    (**sel (core-symbol "DSucc")
				   (**var/def uvar)
				   0)))
	  (alt-where-decls alt))
    (**let `(,(**valdef/def uvar
		(**app (**var/def (core-symbol "coerceB"))
		       (**var/def var)
		       (rconvert-gtype gtype)))
	     ,@(if var? '() (list (**valdef/def var exp))))
	(**and-exp
	 (cfn-ast-1 (**is-constructor (**var/def uvar) (core-symbol "DSucc")))
	 (match-pattern exps pats alt block-name)))))
...
(define (copy-pattern-variables pat)
  (typecase pat
    ...
    (dynamic-pat
      (multiple-value-bind (new-pat vars new-vars)
         (copy-pattern-variables (dynamic-pat-pat pat))
       (values
         (make dynamic-pat
	       (pat new-pat)
	       (sig (dynamic-pat-sig pat))
	       (runtime-vars (dynamic-pat-runtime-vars pat)))
	 vars
	 new-vars)))
    (list-pat
      (multiple-value-bind (new-pats vars new-vars)
