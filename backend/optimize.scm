  ...
  (let ((arg  (car args)))
    (cond ((not (eqv? (dynamic *current-optimize-iteration*)
	  ...
	  (else
	   ...
	   (let ((new-fn
		   (copy-flic-top
		     (or (var-inline-value (core-symbol "inlineBuild"))
			 (var-value (core-symbol "inlineBuild"))
			 (error "Can't find inlineBuild!")))))
	     (if (is-type? 'flic-lambda new-fn)
		 (do-lambda-to-let-aux new-fn args)
		 (values new-fn args))))
	  )))
...
(define (optimize-strict2 fn args)
  (let ((arg1  (car args))
	(arg2  (cadr args)))
    (cond ((is-type? 'flic-return-from arg2)
	   ;; Hoist return-from out of strict2
	   (record-hack 'strict2-hoist-return-from)
	   (let ((exp  (flic-return-from-exp arg2)))
	     (setf (flic-return-from-exp arg2) (make-strict2-app arg1 exp))
	     (values arg2 (cddr args))))
	  ((and (is-type? 'flic-sel arg2)
		(flic-exp-eq? arg1 (flic-sel-exp arg2)))
	   ;; (strict2 exp (sel exp)) => (sel exp)
	...
	(cond ((is-type? 'flic-pack exp)
	       (cond ((eq? (flic-pack-con exp) (core-symbol "True"))
		     ...
		     ((eq? (flic-pack-con exp) (core-symbol "False"))
		      ...
		      ;; Can't replace the whole and expression with false 
		      ;; becauseof possible strictness side-effects.
