...
(define (fully-expand-ntype ty)
  (let ((ty (prune ty)))
    (if (ntyvar? ty)
	ty
	(let ((args (map (function fully-expand-ntype) (ntycon-args ty))))
	  (expand-ntype-synonym (**ntycon (ntycon-tycon ty) args))))))

(define (count-type-arity ty)
  (let ((ty (expand-ntype-synonym ty)))
    (if (or (not (ntycon? ty))
	    (not (eq? (forward-def (ntycon-tycon ty)) (core-symbol "Arrow"))))
	0
	(1+ (count-type-arity (cadr (ntycon-args ty)))))))
...
(define (ntype->gtype/env ntype required-vars)
  (mlet (((gtype env) (ntype->gtype-1 ntype required-vars)))
   (values 
    (make gtype (type gtype)
	  (context (map (lambda (x) (remove-dynamic (ntyvar-context x)))
			env)))
    env)))
