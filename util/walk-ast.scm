...
;;; This is specific to copying ast

(define-syntax (define-copy-walker-methods types)
  `(begin
     ,@(map (function make-copy-walker-method) types)))

(define (make-copy-walker-method ty)
 (let ((type (if (pair? ty) (car ty) ty))
       (ignored-slots (if (pair? ty) (cdr ty) '())))
  `(define-walker-method copy ,type (object env)
     object env ; avoid warnings
     (make ,type
	   ,@(concat
	      (map (lambda (slot)
		     (let ((action (examine-type/copy slot)))
		       (cond ((or (eq? action 'ignore)
				  (memq (sd-name slot) ignored-slots))
			      '())
			     ((eq? action 'share)
			      `((,(sd-name slot)
				 (,(sd-getter slot) object))))
			     ((eq? action 'copy)
			      `((,(sd-name slot)
				 (copy-ast-1 (,(sd-getter slot) object) env))))
			     ((eq? action 'copy-list)
			      `((,(sd-name slot)
				 (copy-ast/list
				  (,(sd-getter slot) object) env)))))))
		     (td-slots (lookup-type-descriptor type))))))))

(define *copied-types* '(exp pattern alt var-ref))
(define *shared-types* '(symbol const integer def signature char string bool))

(define (examine-type/copy slot)
  (let ((ty (sd-type slot)))
    (cond ((memq ty *copied-types*) 'copy)
	  ((and (pair? ty)
		(eq? (car ty) 'list)
		(memq (cadr ty) *copied-types*))
	   'copy-list)
	  ((memq ty *shared-types*)
	   'share)
	  (else
	   'ignore))))
